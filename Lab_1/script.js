...
var massResult = []; // массив который будет содержать обьекты

var obj1 = {
  OutCountries: function() {
    function makeTableFromCountry() {
      for (let i = 0; i < countries.length; i++) {
        var mass1 = countries[i];
        var nonObj = [];
        var hisObj = [];
        var objResult = {
        }   
        ...
        for (let j = 0; j < mass1.length; j++) {
          if (typeof mass1[j] === "object") {
            if (Array.isArray(mass1[j])) {
              for (let l = 0; l < mass1[j].length; l++) {
                if (Array.isArray(mass1[j][l])) {
                  litObj = mass1[j];
                } else {
                  geoObj = mass1[j];
                }
              ...
              nonObj.push(mass1[j][1] + " - " + mass1[j][2]);
              for (var k = 0; k < Object.keys(mass1[j]).length; k++) {
                var key = Object.keys(mass1[j])[k];
                hisObj.push(key + " - " + mass1[j][key]);
              }
            }
          } else {
            if (typeof mass1[j] === "number") {
              nonObj.unshift(mass1[j]);
            } else if (typeof mass1[j] === "string") {
              nonObj.push(mass1[j]);
            }
          }
        }
        objResult[nonObj[1] + "-" + nonObj[2]] = hisObj + "<br>";
        massResult.push(objResult);
      }
    }
...
obj1.OutCountries();
document.write(JSON.stringify(massResult));